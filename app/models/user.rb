class User < ApplicationRecord
  ...
  def pocket_money_update
    ...
    pocket_money_expenses = Expense.where("user_id => ? AND date > ? AND date < ?", id, Time.now, (Time.now + pocket_time.to_s.days))
    pocket_money_expenses.each do |expense|
      u_pocket_money += expense.amount
    end
    pocket_money_event = Event.where("user_id => ? AND date > ? AND date < ?", id, Time.now, (Time.now + pocket_time.to_s.days))
